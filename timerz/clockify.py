...
def split_time_entry(entry):
    start_time = datetime.datetime.fromisoformat(entry["timeInterval"]["start"][:-1]).replace(tzinfo=ZoneInfo(TIMEZONE))
    end_time = datetime.datetime.fromisoformat(entry["timeInterval"]["end"][:-1]).replace(tzinfo=ZoneInfo(TIMEZONE))

    split_start = datetime.datetime.combine(start_time.date(), datetime.time(12, 0), tzinfo=ZoneInfo(TIMEZONE))
    split_end = datetime.datetime.combine(start_time.date(), datetime.time(12, 30), tzinfo=ZoneInfo(TIMEZONE))
...
def get_default_project():
    ...
    return projects[0]["id"] if projects else None # marche pas avec shiroo, certains id sont hidden
...
def add_morning_schedule():
    ...
    start_day = datetime.datetime.combine(today, datetime.time(9, random_minutes), tzinfo=ZoneInfo(TIMEZONE))

    # Create morning meetings
    meetings = [
        {
            "start": datetime.datetime.combine(today, datetime.time(9, 30), tzinfo=ZoneInfo(TIMEZONE)),
            "end": datetime.datetime.combine(today, datetime.time(9, 45), tzinfo=ZoneInfo(TIMEZONE)),
            "description": "Morning Standup"
        },
        {
            "start": datetime.datetime.combine(today, datetime.time(9, 45), tzinfo=ZoneInfo(TIMEZONE)),
            "end": datetime.datetime.combine(today, datetime.time(10, 0), tzinfo=ZoneInfo(TIMEZONE)),
